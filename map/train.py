...
if __name__ == '__main__':
    ...
    #load graph
    graph = json.load(open('graph/svkg.json', 'r'))
    ...
    # A_s + A_v + A_aug + self-loop
    edges = graph['edges'] + [(v, u) for (u, v) in graph['edges']]
    svkg_edges = edges + graph['visual_ed'] + graph['aug_ed'] + [(u, u) for u in range(n)]
    ...
    for epoch in range(1, args.max_epoch + 1):
        ...
        if epoch % args.save_epoch == 0:
            save_checkpoint('svkg-{}'.format(epoch))
